...
self.addEventListener('message', function(e) {
    ...
    [].forEach.call(files, function(file) {
        filesData[file.name] = reader.readAsArrayBuffer(file)
        buffer[file.name]={};
...
function readDataTagData(file,tag,type){
    var b=filesData[file.name].slice(tag.start,tag.end);
    try {
        switch(type) {
            case 2:
...
function readTagData(file, tag) {
    if (tag.type==9) {
        stringcode = new Uint8Array(filesData[file.name].slice(tag.start+4,tag.end))
...
function readTag(file, pos) {
    pos = pos || 8
    var nameL_b = filesData[file.name].slice(pos,pos+4);
    var nameL = new Uint32Array(nameL_b)[0];
    var etr_temp = filesData[file.name].slice(pos+4,pos+4+nameL+20);
